...
public class CameraActivity extends Activity implements RTCEngineEventListener, NetworkStateReceiver.NetworkStateReceiverListener {
    ...
    private boolean rtcCalleeMode = false;
    ...
    public static final String RTC_CALLEE_MODE = "rtc_callee_mode";

    private boolean m_incall = false;
    private final String myID = "1001";
    private final String peerID = "1002";
    //register UI
    private Button m_btnRegister;
    private EditText m_etUserName;
    volatile boolean m_btnRegisterState = false;
    private TextView m_tvRegisterStatus;

    //Call UI
    private Button m_btnCall;
    volatile boolean m_btnCallState = false;
    private EditText m_etRemote;
    private TextView m_tvCallStatus;
    volatile boolean mRTCConnecting = false;
    ...
    private AuthHttpTask ksyRTCAuthTask;
    private KSYOnHttpResponse ksyRTCAuthResponse;
    ...
    public static void startActivity(Context context, int fromType,
                                     ...
                                     boolean testSWFilterInterface, boolean manualFocus, boolean showDebugInfo, boolean isRTCCalleeMode) {
        ...
        intent.putExtra(RTC_CALLEE_MODE, isRTCCalleeMode);
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void handleMessage(Message msg) {
                if (msg != null) {
                    ...
                    switch (msg.what) {
                        ...
                        {
                            String authString = (String)msg.obj;
                            KSYStreamerConfig mStreamerBuilder = mStreamer.getConfig();
                            mStreamerBuilder.setRTCAuthURI(RTC_AUTH_URI)
                                    .setRTCAuthString(authString)
                                    .setRTCUID(m_etUserName.getText().toString())
                                    .setRTCUniqueName(RTC_UINIQUE_NAME)
                                    .setRTCPipLeft(0.6f)
                                    .setRTCPipTop(0.6f)
                                    .setRTCPipWidth(0.4f)
                                    .setRTCPipHeight(0.4f)
                                    .setRTCFps(15)
                                    .setRTCVideoBitrate(256)
                                    .setEnableTLS(true);
                            mStreamer.updateConfig(mStreamerBuilder);
                            int result = mStreamer.registerRTC();
                            if(result == RecorderConstants.RTC_ILLEGAL_STATE) {
                                m_tvRegisterStatus.setText("Register in illegal state");
                                m_btnRegister.setEnabled(true);
                            } else if (result == RecorderConstants.RTC_MISSING_CONFIG) {
                                    m_tvRegisterStatus.setText("Wrong config");
                                    m_btnRegister.setEnabled(true);
                            }
                        }
                        break;
                        ...
                        {
                            m_tvRegisterStatus.setText("Auth failed, pls. try again");
                            m_btnRegister.setEnabled(true);
        ...
        if (bundle != null) {
            ...
            builder.setSampleAudioRateInHz(16000);
            ...
            rtcCalleeMode = bundle.getBoolean(RTC_CALLEE_MODE, false);
            builder.setCalleeMode(rtcCalleeMode);
        ...
        m_btnRegister.setOnClickListener(mObserverButton);
        ...
        m_btnCall.setOnClickListener(mObserverButton);
        m_etRemote = (EditText)findViewById(R.id.etRemoteURI);
        ...
        m_btnAnswer.setOnClickListener(mObserverButton);
        m_btnReject = (Button)findViewById(R.id.btnReject);
        m_btnReject.setOnClickListener(mObserverButton);

        //do not show keyboard autoly
        getWindow().setSoftInputMode(
                WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);
        mStreamer.registerRTCEventListener(this);
        m_etUserName.setText(myID);
        m_etRemote.setText(peerID);
        m_tvConfig = (TextView)findViewById(R.id.tvConfig);

        ksyRTCAuthResponse = new KSYOnHttpResponse() {
            @Override
            public void onHttpResponse(int responseCode, String response) {
                if (responseCode == 200) {
                    mHandler.obtainMessage(RTC_GET_AUTH_INFO_SUCC, response).sendToTarget();
                } else {
                    mHandler.obtainMessage(RTC_GET_AUTH_INFO_FAILED, response).sendToTarget();
                }
            }
        };
    ...
    public void onDestroy() {
        ...
        if (m_btnRegisterState) {
            mStreamer.unregisterRTC();
        }
        if (mStreamer != null) {
            mStreamer.unregisterAllEventListeners();
        }
            ...
            switch (view.getId()) {
                ...
                case R.id.btnRegister:
                    if (m_btnRegisterState == false) {
                        ksyRTCAuthTask =  new AuthHttpTask(ksyRTCAuthResponse);
                        ksyRTCAuthTask.execute(RTC_AUTH_SERVER + "?uid=" + m_etUserName.getText().toString());
                        view.setEnabled(false);
                        m_tvRegisterStatus.setText("Waiting...");
                    }
                    else {
                        if (mStreamer.unregisterRTC() >= 0) {
                            view.setEnabled(false);
                            m_btnRegister.setEnabled(true);
                            m_tvRegisterStatus.setText("OK");
                            ...
                            m_tvRegisterStatus.setText("Unregister in illegal state");
                        }
                    }
                    break;
                case R.id.btnCall:
                    if (m_btnCallState == false) {
                        if (TextUtils.equals(m_etUserName.getText(),m_etRemote.getText())) {
                            m_tvCallStatus.setText("Don't call yourself, pls.");
                            return;
                        }
                        if (mStreamer.startCall(m_etRemote.getText().toString()) < 0) {
                            // startcall in illegal state
                            m_tvCallStatus.setText("Start call in illegal state");
                        } else {
                            view.setEnabled(false);
                        ...
                        if (mStreamer.stopCall() < 0) {
                            m_tvCallStatus.setText("Stop call in illegal state");
                        } else {
                            view.setEnabled(false);
                            m_tvCallStatus.setText("Waiting...");
                        }
                ...
                case R.id.btnAnswer:
                    m_btnCall.setEnabled(true);
                    if (mStreamer.answerCall() < 0) {
                        m_tvCallStatus.setText("Answer call in illegal state");
                    } else {
                        m_tvCallStatus.setText("OK");
                        m_tvIncomming.setVisibility(View.INVISIBLE);
                        m_btnAnswer.setVisibility(View.INVISIBLE);
                ...
                case R.id.btnReject:
                    m_btnCall.setEnabled(true);
                    if (mStreamer.rejectCall() < 0) {
                        m_tvCallStatus.setText("Reject call in illegal state");
                    } else {
                            m_tvIncomming.setVisibility(View.INVISIBLE);
                            m_btnAnswer.setVisibility(View.INVISIBLE);
                            m_btnReject.setVisibility(View.INVISIBLE);
    ...
    @Override
    public void onDataChannelConnectChange(int status) {
        Log.d(TAG,"--------onDataChannelConnectChange status = " + status);
    }
